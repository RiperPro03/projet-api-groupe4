...
const applyParticleEffect = (
  ...
  const appendTextParticle = (
    ...
    content.style.color = options?.particleColor || "currentColor"
  ...
  function generateParticle() {
    ...
    const spinSpeed = (options?.spinSpeed ?? Math.random() * 35) * (Math.random() <= 0.5 ? -1 : 1)
  ...
  function refreshParticles() {
    particles.forEach((p) => {
      p.left = p.left - p.speedHorz * p.direction * animationSpeed
      p.top = p.top - p.speedUp * animationSpeed
      p.speedUp = Math.min(p.size, p.speedUp - gravity * animationSpeed)
      p.spinVal = p.spinVal + p.spinSpeed * animationSpeed
