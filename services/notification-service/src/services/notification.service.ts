...
const CONTENT_RESOURCE_TYPES = new Set<NotificationResourceType>([
  "post",
  "comment",
]);
...
function validateCreateInput(input: CreateNotificationInput) {
  ...
  if (type === "follow") {
    if (resourceType !== "user") {
      throw new NotificationError(
        "resourceType doit être user pour une notification follow",
        400
      );
    }
  ...
  } else if (!CONTENT_RESOURCE_TYPES.has(resourceType)) {
    throw new NotificationError(
